...
class EDVCdataset(Dataset):
    ...
    def __init__(self, anno_file, feature_folder, translator_json, is_training, proposal_type, opt):
        ...
        # this file is for only for evaluation of old runs
        # this file is keep because we need it for previous runs when running eval_old.sh
        ...
        opt.need_sample_for_multi = vars(opt).get('need_sample_for_multi', False)
        opt.eval_for_multi_anno = vars(opt).get('eval_for_multi_anno', False)

        self.need_sample_for_multi = opt.need_sample_for_multi and is_training
        self.need_rebuild_feat_key = (opt.eval_for_multi_anno and not is_training) or (opt.train_with_split_anno and is_training)
...
class PropSeqDataset(EDVCdataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.need_sample_for_multi:
            gt_timestamps, captions = sample_for_multi_sentence(gt_timestamps, captions, sample_events=self.need_sample_events,
...
def sample_for_multi_sentence(gt_timesteps, sentences, sample_events=False, min_sample_num=5, max_sample_num=10):
    timesteps_dict = {}
    sample_res = {}
