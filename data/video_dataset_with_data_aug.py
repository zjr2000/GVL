...
class EDVCdataset(Dataset):
    ...
    def __init__(self, anno_file, feature_folder, translator_json, is_training, proposal_type, opt):
        ...
        self.need_sample_for_multi = (opt.dataset == 'tacos' or opt.dataset == 'char') and is_training
        self.need_rebuild_feat_key = (opt.dataset == 'tacos' or opt.dataset == 'char') and not is_training
...
class PropSeqDataset(EDVCdataset):
    ...
    def load_feats_and_rescale(self, key):
        ...
        if type(vf_types) == list:
            ...
            if len(out) > self.opt.frame_embedding_num:
                ...
                crop_start_ratio, crop_end_ratio = 0.0, 1.0
            crop_start = int((len(out) - 1) * crop_start_ratio)
            crop_end = int((len(out) - 1) * crop_end_ratio)
    ...
    def _get_item_helper(self, idx):
        ...
        while True:
            ...
            raw_duration = self.anno[key]['duration']
            raw_captions = self.anno[key]['sentences']
            raw_gt_timestamps = self.anno[key]['timestamps']  # [gt_num, 2]

            if self.need_sample_for_multi:
                raw_gt_timestamps, raw_captions = sample_for_multi_sentence(raw_gt_timestamps, raw_captions)

            dur_ratio = end_ratio - start_ratio
            duration = raw_duration * dur_ratio
            start = start_ratio * raw_duration
            end = end_ratio * raw_duration
            gt_timestamps_ = [[max(ts[0], start) - start, min(ts[1], end) - start] for ts in raw_gt_timestamps]
            gt_timestamps = []
            captions = []
            for i, ts in enumerate(gt_timestamps_):
                if ts[1] - ts[0] <= 0:
                    continue
                if (ts[1] - ts[0]) / (raw_gt_timestamps[i][1] - raw_gt_timestamps[i][0] + 1e-5) < 1./3:
                    continue
    ...
    def __getitem__(self, idx):
        r = list(zip(*[self._get_item_helper(idx) for i in range(self.opt.crop_num)]))
        # pdb.set_trace()
        return r


def sample_for_multi_sentence(gt_timesteps, sentences):
    timesteps_dict = {}
    sample_res = {}
