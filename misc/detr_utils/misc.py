...
class AllGather_multi(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, tensor, num_list):
        ctx.rank = get_rank()
        ctx.world_size = get_world_size()
        start = 0
        for i in range(get_rank()):
            start += num_list[i]
        length = num_list[get_rank()]
        ctx.start = start
        ctx.length = length

        max_len = max(num_list)
        # make sure different rank has the same dim, otherwise all_gather will corrupt
        padded_shape = list(tensor.shape)
        padded_shape[0] = max_len - padded_shape[0] 
        ...
        # Remove padding
        tensor_list = [tensor_list[i][:num] for i, num in enumerate(num_list)]
        return torch.cat(tensor_list, dim=0)

    @staticmethod
    def backward(ctx, grad_output):
        return (
        grad_output[ctx.start : ctx.start + ctx.length],
        None, None,
    )
...
def reduce_dict_numpy(dict):
    for key, val in dict.items():
        val = torch.tensor([val]).to(get_rank())
        dist.all_reduce(val, op=dist.ReduceOp.SUM)
        val = val[0].cpu() / get_world_size()
        dict[key] = val.item()
    return dict
