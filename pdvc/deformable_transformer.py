...
class DeformableTransformer(nn.Module):
    ...
    def prepare_decoder_input_proposal(self, gt_reference_points, inversed_input=False):
        if inversed_input:
            topk_coords_unact = gt_reference_points
            gt_reference_points = torch.sigmoid(gt_reference_points)
        else:
    ...
    def convert_proposal_to_query(self, gt_reference_points):
        topk_coords_unact = inverse_sigmoid(gt_reference_points)
        query = self.pos_trans_norm(self.pos_trans(self.get_proposal_pos_embed(topk_coords_unact)))
        return query
